...
<template>
  <div class="player-container">
    <h2>
      ...
      <span v-if="store.activeEffects.luckySeven" class="lucky-seven-indicator">
        🍀 Счастливая семёрка</span
      >
      <span v-if="store.activeEffects.chronometer > 0" class="chronometer-indicator">
        ⏰ Хронометр ({{ store.activeEffects.chronometer }})</span
      >
      <span v-if="store.activeEffects.royalDecree" class="royal-decree-indicator">
        👑 Королевский указ (+2 к каждой карте)</span
      >
    ...
    <div :class="getHandContainerClass(hand.length)">
      ...
      >
        ...
        <div v-if="card.value === 'A' && store.activeEffects.aceArmor" class="ace-armor-badge">
          🛡️
        </div>
        <div v-if="card.value === 'A' && store.activeEffects.fireAce" class="fire-ace-badge">
          🔥
        </div>
        <div v-if="card.value === '7' && store.activeEffects.luckySeven" class="lucky-seven-badge">
          🍀
        </div>
