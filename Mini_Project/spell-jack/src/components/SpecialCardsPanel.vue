...
function getCardStatus(card: SpecialCard): string {
  const statusMap: Record<string, () => string> = {
    doubleNext: () => (store.activeEffects.doubleNext ? 'Готов к удвоению!' : 'Нажмите для активации'),
    dealerTrap: () => (store.activeEffects.dealerTrap ? 'Ловушка установлена!' : 'Нажмите для активации'),
    ...
    stabilizer: () => (store.activeEffects.stabilizer ? 'Коэффициенты x1.0!' : 'Убрать случайность!'),
    goldenTouch: () => (store.activeEffects.goldenTouch ? 'Готов к золоту!' : 'Монеты = очки!'),
    chronometer: () => (store.activeEffects.chronometer > 0 ? `Осталось ${store.activeEffects.chronometer} карт!` : 'Замедлить время!'),
    suitMagnet: () => (store.activeEffects.suitMagnetActive ? 'Выберите масть!' : 'Усилить масть +1!'),
...
function getPassiveStatus(card: SpecialCard): string {
  const statusMap: Record<string, () => string> = {
    ...
    luckySuit: () => (store.activeEffects.luckySuitActive ? `Усилена ${store.activeEffects.luckySuitActive}!` : 'Пассивный эффект'),
    royalDecree: () => (store.activeEffects.royalDecree ? 'Все карты +2 очка!' : 'Пассивный эффект'),
  }
...
function getCardClass(card: SpecialCard): string {
  ...
  const classMap: Record<string, () => string> = {
    ...
    stabilizer: () => (store.activeEffects.stabilizer ? `${base} stabilizer-effect active` : `${base} stabilizer-effect`),
    goldenTouch: () => (store.activeEffects.goldenTouch ? `${base} golden-touch-effect active` : `${base} golden-touch-effect`),
    chronometer: () => (store.activeEffects.chronometer > 0 ? `${base} chronometer-effect active` : `${base} chronometer-effect`),
    suitMagnet: () => (store.activeEffects.suitMagnetActive ? `${base} suit-magnet-effect active` : `${base} suit-magnet-effect`),
...
<template>
  ...
  >
    ...
    <template v-if="passiveCards().length > 0">
      ...
      <div class="special-cards-grid">
        <div
          v-for="card in passiveCards()"
          :key="card.id"
          :class="getPassiveCardClass(card)"
        >
          <div class="card-emoji">{{ card.value }}</div>
          <div class="card-name">{{ card.name }}</div>
