    ...
    setTimeout(() => {
      ...
      if (newCard.special) {
        ...
        if (result?.requiresDestinyPreview) {
          ...
          if (next) {
            destinyPreview.value = store.previewNextCardOutcome(playerHand.value, playerScore.value, next)
      ...
      if (store.activeEffects.doubleNext && !newCard.special) {
        ...
        showTemporaryMessage(`⚡ Двойной удар! ${newCard.value}${newCard.suit} дает +${bonus} бонусных очков!`)
        haptic('success')
      ...
      if (store.activeEffects.goldenTouch && !newCard.special) {
        ...
        showTemporaryMessage(`✨ Золотое касание! ${newCard.value}${newCard.suit} дает +${finalValue} монет!`)
        haptic('success')
