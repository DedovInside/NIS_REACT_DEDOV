  ...
  getters: {
    manualActivationCards(state): SpecialCard[] {
      return state.activeSpecialCards.filter(
        card =>
          card.activationType === 'manual' && !state.usedSpecialEffects.includes(String(card.id)),
      );
    },
  },
  ...
  actions: {
    ...
    applyAutoEffects() {
      this.activeSpecialCards
        .filter(
          card =>
            card.activationType === 'auto' && !this.usedSpecialEffects.includes(String(card.id)),
        )
        .forEach(card => {
    ...
    checkPassiveEffects(context: string) {
      this.activeSpecialCards
        .filter(
          card =>
            card.activationType === 'passive' && !this.usedSpecialEffects.includes(String(card.id)),
        )
        .forEach(card => {
